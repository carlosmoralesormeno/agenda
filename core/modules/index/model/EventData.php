...
class EventData {
	...
	public function add(){
		$sql = "insert into event (title,description,project_id,date_at,time_at,time_end,category_id,user_id,created_at) ";
		 $sql .= "value (\"$this->title\",\"$this->description\",$this->project_id,\"$this->date_at\",\"$this->time_at\",\"$this->time_end\",$this->category_id,$this->user_id,$this->created_at)";
...
// partiendo de que ya tenemos creado un objecto EventData previamente utilizamos el contexto
	public function update(){
		$sql = "update ".self::$tablename." set title=\"$this->title\",project_id=$this->project_id,category_id=$this->category_id,date_at=\"$this->date_at\",time_at=\"$this->time_at\",time_end=\"$this->time_end\",description=\"$this->description\" where id=$this->id";
		Executor::doit($sql);
	}
